# scripts/preprocess/aux_spade.py
# ----------------------------
...
def resize_patch(patch, size):
    ...
    for i in range(c):
        ...
        out[i] = cv2.resize(patch[i], (size, size), interpolation=interp)
    return out
...
for tile_name, info in metadata.items():
    ...
    aux_path = AUX_DIR / f"{campaign}_aux_epsg2154.tif"

    mid_path = PATCHES_DIR / split / "AUX_SPADE_MID" / f"{tile_name}.npy"
    hr_path = PATCHES_DIR / split / "AUX_SPADE_HR" / f"{tile_name}.npy"
    ...
    if not aux_path.exists():
        ...
        np.save(mid_path, np.zeros((20, MID_SIZE, MID_SIZE), dtype=np.float32))
        np.save(hr_path, np.zeros((20, HR_SIZE, HR_SIZE), dtype=np.float32))
        continue
    ...
    # ----------------------------
    # WINDOW (FIX: Direct reading using metric bounds—No transformer shift)
    # ----------------------------
    window = from_bounds(
        bounds["left"],
        bounds["bottom"],
    ...
    try:
        ...
        # FIX: Index layers read with nearest neighbor to preserve sharp physical river interfaces
        ndvi = src.read(
            6,
            window=window,
    ...
    lulc_onehot = encode_lulc_onehot(fixed)
    ...
    aux_hr = np.nan_to_num(aux_hr, nan=0.0, posinf=1.0, neginf=0.0)

    # ----------------------------
    # MID FOR SPADE
    # ----------------------------
    aux_mid = resize_patch(aux_hr, MID_SIZE)

    # ----------------------------
    # SAVE
    # ----------------------------
    np.save(hr_path, aux_hr.astype(np.float32))
    np.save(mid_path, aux_mid.astype(np.float32))
